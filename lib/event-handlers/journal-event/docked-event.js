...
module.exports = (payload) => {
  ...
  // We could catch these by checking for 'SpaceConstructionDepot' or 'PlanetaryConstructionDepot'
  // but I'm happy to continue to log them for now. I am not entirely certain if their Market ID
  // stays the same when the construction is complete (in which case it's fine to leave them, 
  // as the Station Type will be updated automatically when someone docks with it) or if the 
  // newly constructed location gets a new Market ID (in which case it would be best to purge
  // these sites after something like X days of inactivity). I should be able to determine this
  // myself by checking what happens in the journal before and after completing a new location.
